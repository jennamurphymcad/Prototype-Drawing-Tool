...
function draw() {
  ...
  background(0);
for (x = 20; x <= 1000; x = x+ 3)//slider1
  for (y = 20; y <= 300; y = y +10) {

    noFill();
    beginShape();
stroke(0);
    strokeWeight(.1);
    vertex(200, 0); //slider2
    quadraticVertex(x/6, y, 255, 100);//slider3 and 4
    endShape();

    beginShape();
    strokeWeight(.1);
    vertex(100, 100); //slider5
    quadraticVertex(x/PI, y/HALF_PI, 0, 100);
    endShape();
    stroke(255);
    beginShape();
    strokeWeight(0.2);
    vertex(300, 0); //slider6
    quadraticVertex(x, y, 100, 400);
    endShape();
    rotate(x*30); //slider7

    translate(-30, 50); //slider9
